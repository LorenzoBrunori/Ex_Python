...
mask_array = dow[:, VOLUME] > 5500000000
# 2. How many are there?  (hint: use sum).
_sum = sum(mask_array)
print('%d days: ', mask_array)
# 3. Find the index of every row (or day) where the volume is greater
#    than 5.5 billion. hint: look at the where() command.
index = where(mask_array)[0]
print('Index: ', index)
# BONUS:
...
axes.plot(dow[:, ADJ_CLOSE], color = 'black', label = 'adjusted close for EVERY day')
axes.plot(index, dow[index, ADJ_CLOSE], '.', color = 'red', label = 'volume was greater than 5.5 billion')

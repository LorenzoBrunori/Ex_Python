...
print('1) ex')
return_value = ((prices[1:] - prices[:-1]) / prices[:-1])
print('Return_Value: ', return_value)


zero_line = zeros(len(return_value))
fig, axes = plt.subplots()
axes.plot(zero_line, color = 'red')
axes.plot(return_value * 100, '.',color = 'blue', label = 'Return Value')

print("Prices for AAPL stock in 2008:")
print(prices)
axes.legend()
plt.xlim(xmax=len(return_value))
plt.show()